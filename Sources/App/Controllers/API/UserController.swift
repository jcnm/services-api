...
final class UserController {
  ...
  public func login(_ req: Request) throws -> Future<User.FullLoggedResponse> {

      let logUser = try? loginForm(req)
  ...
  /// Logs a user in, returning a token for accessing protected endpoints.
  public func loginForm(_ req: Request) throws -> Future<User.FullLoggedResponse> {
    // get user auth'd by basic auth middleware
    let usr = try req.content.decode(User.Login.self)
    return usr.flatMap{ (usr) -> EventLoopFuture<User.FullLoggedResponse> in
      ...
      return User.authenticate(
        ...
        .flatMap { user in
          ...
          let logger = try req.make(Logger.self)
          logger.info("\(u.email) - \"\(u.login)\" is connected for a Session")
          let token = try UserToken.create(userID: u.requireID())
          logger.info("\(String(describing: u.id)) - \"\(u.login)\" has a new token witch expire on \(String(describing: token.expiresOn))")
          return token.save(on: req).flatMap { tok in // Log the saving token
            logger.info("\(String(describing: u.id)) - \"\(String(describing: u.login))\" token \"\(String(describing: token.id))\" for user \"\(token.user)\" saved")
            /// TODO detect agent first ! If web browser, then create session
            try req.authenticateSession(u)
            try req.session()["login"] = u.login
            try req.session()["profileID"] = String(u.profileID!)
            try req.session()["httpOnly"] = "false"
            logger.info("Session information - \"\(try req.session().data))\"")
            return u.fullLoggedResponse(req, tok)
          }
      }
    }
  }
...
extension UserController: RouteCollection {
  func boot(router: Router) throws {
    ...
    basic.post(kUsersLoginBasePath, use: login)
