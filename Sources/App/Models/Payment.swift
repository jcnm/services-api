...
/// Seed for OrderItem
struct SeedPayment: Migration {
  typealias Database = AdoptedDatabase
  
  static func prepare(on connection: AdoptedConnection) -> Future<Void> {
    let pymnt0 = Payment(user: 5, organization: 3)
    pymnt0.token = Utils.newRef("PYMTEST", size: 32, on: nil)
    let pymnt1 = Payment(user: 5, organization: 7, card: 1)

    _ = pymnt0.save(on: connection).catch({ (e) in
      print("ERROR PAYEMENT----------")
      print(e)
    }).transform(to: ())
    _ = pymnt1.save(on: connection).catch({ (e) in
      print("ERROR PAYEMENT1----------")
      print(e)
    }).transform(to: ())
