...
/// Seed for Asset
struct SeedAsset: Migration {
  typealias Database = AdoptedDatabase

  static func prepare(on connection: AdoptedConnection) -> Future<Void> {
    
    // For Asset 1
    let ass1 = Asset(author: 7, organization: 27, description: "Les établissements dont 50 % ou plus des activités se rapportent à l'élevage d'animaux ou l'aquaculture, mais dont aucun produit ou famille de produits ne représente 50 % de la production, sont considérés comme des fermes d'élevage mixte et figurent à la rubrique 11299 Tous les autres types d'élevage", state: .defaultValue, cost: 2.99, forServices: [])
    let ass2 = Asset(author: 2, organization: 17, description: "Some asset second", state: .defaultValue, cost: 0.99, forServices: [])
    let ass3 = Asset(author: Config.Static.bbMainUserID, organization: Config.Static.bbMainOrgID, description: "Asset for listed service", state: .online, cost: -10.99, forServices: [3,4,8,9])
    let ass4 = Asset(author: Config.Static.bbMainUserID, organization: Config.Static.bbMainOrgID, description: "Empty asset for main org", state: .defaultValue, cost: 5.99, forServices: [])
    let ass5 = Asset(author: Config.Static.bbMainUserID, organization: Config.Static.bbMainOrgID, description: "A dictionary is a type of hash table, providing fast access to the entries it contains", state: .defaultValue, cost: 4.99, forServices: [])
    let ass6 = Asset(author: 4, organization: 20, description: "A PIVOT relational operator is used to convert values of multiple rows into values", state: .defaultValue, cost: 2.99, forServices: [])
    let ass7 = Asset(author: 6, organization: 17, description: "Les traditions de Pâques. Jonquilles et oeufs de Pâques - Photo by Monika Grabkowska on Unsplash. L'oeuf, symbole de", state: .defaultValue, cost: 0.99, forServices: [])
    let ass8 = Asset(author: Config.Static.bbMainUserID, organization: Config.Static.bbMainOrgID, description: "Les établissements inclus dans ces classes peuvent utiliser des méthodes de cultures agricoles traditionnelles", state: .online, cost: -40.99, forServices: [3,4,7, 8, 9])
    let ass9 = Asset(author: 9, organization: Config.Static.bbMainOrgID, description: "Les établissements dont l'activité principale est la production de semences sont rangés dans la classe de la culture appropriée", state: .defaultValue, cost: 35.99, forServices: [6, 7,8])
    let ass10 = Asset(author: 2, organization: Config.Static.bbMainOrgID, description: "Ce groupe comprend les établissements dont l'activité principale est l'élevage de porcs", state: .defaultValue, cost: 15.99, redeem: true, redeemCode: "REDCR20", forUsers: [2, 7, 4], forServices: [])

    _ = ass1.save(on: connection).catch({ (e) in
      print("ERROR SERVICE ASSET 1 ----------")
      print(e)
    }).transform(to: ())
    _ = ass2.save(on: connection).catch({ (e) in
      print("ERROR SERVICE ASSET 2 ----------")
      print(e)
    }).transform(to: ())
    _ = ass3.save(on: connection).catch({ (e) in
      print("ERROR SERVICE ASSET 3 ----------")
      print(e)
    }).transform(to: ())
    
    _ = ass4.save(on: connection).transform(to: ())
    _ = ass5.save(on: connection).transform(to: ())
    _ = ass6.save(on: connection).transform(to: ())
    _ = ass7.save(on: connection).transform(to: ())
    _ = ass8.save(on: connection).transform(to: ())
    _ = ass9.save(on: connection).transform(to: ())
    _ = ass10.save(on: connection).transform(to: ())
