...
struct SeedPlace: Migration {
  typealias Database = AdoptedDatabase
  
  static func prepare(on connection: AdoptedConnection) -> Future<Void> {
    let place1 = Place(label: nil, number: "2", kind: PlaceKind.street, street: "Some Street", city: "Some City", state: "Some state", postalCode: "Some postalCode", country: "France")
    let paddr = Place(label: nil, number: "95", kind: PlaceKind.street, street: "du capitaine guynemer", city: "Courbevoie", state: "Ile De France", postalCode: "92400", country: "France", position: [48.912788, 2.2887033])
    let place2 = Place(label: nil, number: "4", kind: PlaceKind.place, street: "De La Défense", city: "Courbevoie", state: "Ile de france", postalCode: "92400", country: "France")
    let place3 = Place(label: nil, number: "42", kind: PlaceKind.avenue, street: "Marceau", city: "Courbevoie", state: "Ile De France", postalCode: "92400", country: "France", position: [48.912788, 2.2887033])
    _ = place1.create(on: connection).transform(to: ())
    _ = place2.create(on: connection).transform(to: ())
    _ = place3.create(on: connection).transform(to: ())
    return paddr.create(on: connection).transform(to: ())
  }
