...
struct SeedContact: Migration {
  typealias Database = AdoptedDatabase
  
  static func prepare(on connection: AdoptedConnection) -> Future<Void> {
    let cont1 = Contact(givenName: "Edouard", familyName: "Ma famille", emailAddresses: [], id: 10)
    cont1.emailAddresses?.append(NamedEmail(label: "work", value: "some@sd.fr"))
    cont1.emailAddresses?.append(NamedEmail(label: "home", value: "some@gmail.com"))
    
    let cont2 = Contact(givenName: "Phillip", familyName: "De Pelardouh", urlAddresses: [], places: [], jobTitle: "Lead Developer", birthday: Date(rfc1123: "Sun, 06 Nov 1994 08:49:37 GMT") )
    cont2.urlAddresses?.append(NamedURI(label: "corp", value: "services.cm"))
    cont2.phoneNumbers?.append(NamedURI(label: "home", value: "+123578654"))
    ...
    let cont3 = Contact(givenName: nil, familyName: nil, nickname: "Sylorion Bourg La Reine", ckind: .organization(.sarl), places: [])
    cont3.places?.append(Place(label: "To reconsider", number: "93", kind: .street, street: "du capitaine guynemer", city: "Montrouge", state: "Ile de france", postalCode: "92120", country: "France"))
    let cont4 = Contact(givenName: "Qarlif", familyName: "Trya Meguez", emailAddresses: [], jobTitle: "Ingénieur de recherche")
    cont4.emailAddresses?.append(NamedEmail(label: "work", value: "some-email@gmail.fr"))
    cont4.emailAddresses?.append(NamedEmail(label: "home", value: "qsdfg-zdfv@gmail.com"))
    
    let cont5 = Contact(givenName: "Wellner", familyName: "Derruos", urlAddresses: [], places: [], jobTitle: "Senior Developer", birthday: Date(rfc1123: "Sun, 06 Nov 1994 08:49:37 GMT") )
    cont5.urlAddresses?.append(NamedURI(label: "Travail", value: "organization.org.cm"))
    cont5.phoneNumbers?.append(NamedURI(label: "Maison", value: "+123578654"))
    cont5.places?.append(Place(label: "To complet", number: "95", kind: .street, street: "Du capitaine", city: "Anvoirie", state: "Acquitaine", postalCode: "65400", country: "France"))
    
    let cont6 = Contact(givenName: "Amour de dieux", familyName: "nil", nickname: "Sylorion Bourg La Reine", ckind: .person(.female), places: [])
    cont6.places?.append(Place(label: "Quelque Part", number: "13", kind: .street, street: "du Paradis", city: "Montrouge", state: "Ile de france", postalCode: "92120", country: "France"))
    
    _ = cont1.create(on: connection).transform(to: ())
    _ = cont2.create(on: connection).transform(to: ())
    _ = cont3.create(on: connection).transform(to: ())
    
    _ = cont4.create(on: connection).transform(to: ())
    _ = cont5.create(on: connection).transform(to: ())
    _ = cont6.create(on: connection).transform(to: ())
