...
/// Seed for BankCard
struct SeedContract: Migration {
  typealias Database = AdoptedDatabase
  
  static func prepare(on connection: AdoptedConnection) -> Future<Void> {
    let ctr0 = Contract(label: "Ideom/Edrt 19-1", order: 3, object: "Blabla sur l'obj", execution: "Blabla execution rules", duration: "blabla duration rule", payment: "blabla payment rules", endContract: "blabla end of contract rule", litige: "blabla litige rules", lang: "FR-fr")

    _ = ctr0.save(on: connection).transform(to: ())
    let ctr1 = Contract(label: "ORG1/ORG 19-1", order: 2, object: "Blabla sur l'obj", execution: "Blabla execution rules", duration: "blabla duration rule", payment: "blabla payment rules", endContract: "blabla end of contract rule", litige: "blabla litige rules", lang: "EN-gb")

    _ = ctr1.save(on: connection).catch({ (e) in
      print("ERROR CONTRACT1----------")
      print(e)
    }).transform(to: ())
